...
printf "%s\n" '*/' >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" '#import <Cocoa/Cocoa.h>' >> "${DestFile}"
printf "%s\n" '#include <CoreAudio/CoreAudio.h>' >> "${DestFile}"
printf "%s\n" '#include <AudioUnit/AudioUnit.h>' >> "${DestFile}"
printf "%s\n" '#include <stdio.h>' >> "${DestFile}"
printf "%s\n" '#include <stdlib.h>' >> "${DestFile}"
printf "%s\n" '#include <string.h>' >> "${DestFile}"
printf "%s\n" '#include <sys/param.h>' >> "${DestFile}"
printf "%s\n" '#include <sys/time.h>' >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" '#define MyAppIsBundle 1' >> "${DestFile}"
printf "%s\n" '#define WantUnTranslocate 0' >> "${DestFile}"
printf "\n" >> "${DestFile}"
...
DestFile="${my_config_d}Info.plist"
printf "" > "${DestFile}"

printf "%s\n" '<?xml version="1.0" encoding="UTF-8"?>' >> "${DestFile}"
printf "%s\n" '<!DOCTYPE plist PUBLIC "-//Apple Computer//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">' >> "${DestFile}"
printf "%s\n" '<plist version="1.0">' >> "${DestFile}"
printf "%s\n" '<dict>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleDevelopmentRegion</key>' >> "${DestFile}"
printf "%s\n" '	<string>English</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleDocumentTypes</key>' >> "${DestFile}"
printf "%s\n" '	<array>' >> "${DestFile}"
printf "%s\n" '		<dict>' >> "${DestFile}"
printf "%s\n" '			<key>CFBundleTypeOSTypes</key>' >> "${DestFile}"
printf "%s\n" '			<array>' >> "${DestFile}"
printf "%s\n" '				<string>****</string>' >> "${DestFile}"
printf "%s\n" '			</array>' >> "${DestFile}"
printf "%s\n" '			<key>CFBundleTypeRole</key>' >> "${DestFile}"
printf "%s\n" '			<string>Editor</string>' >> "${DestFile}"
printf "%s\n" '		</dict>' >> "${DestFile}"
printf "%s\n" '	</array>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleExecutable</key>' >> "${DestFile}"
printf "%s\n" '	<string>minivmac</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleGetInfoString</key>' >> "${DestFile}"
printf "%s\n" '	<string>minivmac-36.04-mc64, Copyright 2018 maintained by unknown.</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleIconFile</key>' >> "${DestFile}"
printf "%s\n" '	<string>ICONAPPO.icns</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleIdentifier</key>' >> "${DestFile}"
printf "%s\n" '	<string>com.gryphel.minivmac</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleInfoDictionaryVersion</key>' >> "${DestFile}"
printf "%s\n" '	<string>6.0</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleName</key>' >> "${DestFile}"
printf "%s\n" '	<string>Mini vMac</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundlePackageType</key>' >> "${DestFile}"
printf "%s\n" '	<string>APPL</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleShortVersionString</key>' >> "${DestFile}"
printf "%s\n" '	<string>36.04</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleSignature</key>' >> "${DestFile}"
printf "%s\n" '	<string>????</string>' >> "${DestFile}"
printf "%s\n" '	<key>CFBundleVersion</key>' >> "${DestFile}"
printf "%s\n" '	<string>36.04</string>' >> "${DestFile}"
printf "%s\n" '	<key>LSRequiresCarbon</key>' >> "${DestFile}"
printf "%s\n" '	<string>1</string>' >> "${DestFile}"
printf "%s\n" '	<key>NSHighResolutionCapable</key>' >> "${DestFile}"
printf "%s\n" '	<string>1</string>' >> "${DestFile}"
printf "%s\n" '</dict>' >> "${DestFile}"
printf "%s\n" '</plist>' >> "${DestFile}"
...
printf "\n" >> "${DestFile}"
printf "%s\n" 'TheDefaultOutput : minivmac.app/Contents/MacOS/minivmac' >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" 'bld/OSGLUCCO.o : src/OSGLUCCO.m src/STRCNENG.h cfg/STRCONST.h src/INTLCHAR.h src/COMOSGLU.h src/CONTROLM.h cfg/CNFGGLOB.h' >> "${DestFile}"
printf "%s\n" '	gcc "src/OSGLUCCO.m" -o "bld/OSGLUCCO.o" $(mk_COptions)' >> "${DestFile}"
...
printf "\n" >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" 'minivmac.app/Contents/Resources/ICONAPPO.icns : src/ICONAPPO.icns' >> "${DestFile}"
printf "%s\n" '	rm -fr "minivmac.app/"' >> "${DestFile}"
printf "%s\n" '	rm -fr "AppTemp/"' >> "${DestFile}"
printf "%s\n" '	mkdir "AppTemp/"' >> "${DestFile}"
printf "%s\n" '	mkdir "AppTemp/Contents/"' >> "${DestFile}"
printf "%s\n" '	mkdir "AppTemp/Contents/MacOS/"' >> "${DestFile}"
printf "%s\n" '	mkdir "AppTemp/Contents/Resources/"' >> "${DestFile}"
printf "%s\n" '	mkdir "AppTemp/Contents/Resources/English.lproj/"' >> "${DestFile}"
printf "%s\n" '	cp "src/ICONAPPO.icns" "AppTemp/Contents/Resources/"' >> "${DestFile}"
printf "%s\n" '	cp "cfg/Info.plist" "AppTemp/Contents/"' >> "${DestFile}"
printf "%s\n" '	cp "src/shaders.metal" "AppTemp/Contents/Resources//shaders.metal" 2>/dev/null || true' >> "${DestFile}"
printf "%s\n" '	printf "dummy\n" > "AppTemp/Contents/Resources/English.lproj/dummy.txt"' >> "${DestFile}"
printf "%s\n" '	printf "APPL????" > "AppTemp/Contents/PkgInfo"' >> "${DestFile}"
printf "%s\n" '	mv "AppTemp/" "minivmac.app/"' >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" 'minivmac.app/Contents/MacOS/minivmac : $(ObjFiles) minivmac.app/Contents/Resources/ICONAPPO.icns' >> "${DestFile}"
printf "%s\n" '	gcc \' >> "${DestFile}"
printf "%s\n" '		-o "minivmac.app/Contents/MacOS/minivmac" \' >> "${DestFile}"
printf "%s\n" '		$(ObjFiles) -framework AppKit -framework AudioUnit -framework Metal -framework QuartzCore' >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" 'clean :' >> "${DestFile}"
